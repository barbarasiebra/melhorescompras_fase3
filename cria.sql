...
COMMENT ON COLUMN mc_bairro.cd_bairro IS
    'Esta coluna ira receber o codigo do bairro e seu conteudo é obrigatório.';

COMMENT ON COLUMN mc_bairro.cd_cidade IS
    'Esta coluna ira receber o codigo da cidade e seu conteudo é obrigatório.';

COMMENT ON COLUMN mc_bairro.nm_bairro IS
    'Esta coluna ira receber o nome do Bairro. Esse conteudo é obrigatório.';

COMMENT ON COLUMN mc_bairro.nm_zona_bairro IS
    'Esta coluna ira receber a localização da zona onde se encontra o bairro. Alguns exemplos: Zona Norte, Zona Sul, Zona Leste, Zona Oeste, Centro.';
...
COMMENT ON COLUMN mc_categoria_prod.cd_categoria IS
    'Essa coluna ira armazenar a chave primária da tabela de categoria de produtos da Melhorees Compras. Cada categoria nova cadastrada  será acionada a Sequence  SQ_MC_CATEGORIA que se encarregará de gerar o próximo número único da categoria..';

COMMENT ON COLUMN mc_categoria_prod.tp_categoria IS
    'Nessa  coluna  será  armazenada o tipo de categoria que poderá  ser (V)ídeo ou (P)rodudto. Seu conteudo deve ser obrigatório.';

COMMENT ON COLUMN mc_categoria_prod.ds_categoria IS
    'Essa coluna ira armazenar descrição da categoria de produtos da Melhorees Compras. Cada categoria tem uma  descrição única e serve para organizar os produtos em categorias simliares, melhorando a tomada de decisão.';

COMMENT ON COLUMN mc_categoria_prod.dt_inicio IS
    'Essa coluna ira receber  a data de início da categoria. Seu formato é dd/mm/yyyy e seu conteudo deve ser sempre  obrigatório.';

COMMENT ON COLUMN mc_categoria_prod.dt_termino IS
    'Essa coluna ira receber  a data de encerramento  da categoria. Seu formato é dd/mm/yyyy e seu conteudo deve ser sempre  opcional. conteudo obrigatório significa que a categoria foi encerrada a partir da data término. Já o conteudo opcional indica que a categoria está ativa e operante.';

COMMENT ON COLUMN mc_categoria_prod.st_categoria IS
    'Essa coluna ira armazenar o stauts da categoria da Melhores Compras. Os valores permitidos aqui são: A(tivo) e I(nativo).';
...
COMMENT ON COLUMN mc_cidade.sg_estado IS
    'Esta coluna ira receber a siga do Estado. Esse conteudo é obrigatório.';

COMMENT ON COLUMN mc_cidade.nm_cidade IS
    'Esta coluna ira receber o nome da Cidade. Esse conteudo é obrigatório.';

COMMENT ON COLUMN mc_cidade.cd_ibge IS
    'Esta coluna ira receber o codigo do IBGE que fornece informações para geração da NFe.';

COMMENT ON COLUMN mc_cidade.nr_ddd IS
    'Esta coluna ira receber o número do DDD da cidade para ser utilizado no contato telefônico. Seu conteudo é opcional.';
...
COMMENT ON COLUMN mc_cli_fisica.nr_cliente IS
    'Essa coluna ira armazenar o codigo único do cliente na plataforma ecommerce da Melhores Compras.Seu conteudo deve ser obrigatório, único e preenhcido a  parrtir da tabela NC_CLIENTE. ';

COMMENT ON COLUMN mc_cli_fisica.dt_nascimento IS
    'Essa coluna ira armazenar a data de nascimento do cliente na plataforma ecommerce da Melhores Compras.Seu conteudo deve ser obrigatório. ';

COMMENT ON COLUMN mc_cli_fisica.fl_sexo_biologico IS
    'Essa coluna ira armazenar o sexo biológico do cliente na plataforma ecommerce da Melhores Compras.Seu conteudo deve ser obrigatório.. ';

COMMENT ON COLUMN mc_cli_fisica.ds_genero IS
    'Genero do sexo do Cliente. Seu conteudo deve ser obrigatório.';

COMMENT ON COLUMN mc_cli_fisica.nr_cpf IS
    'Essa coluna ira armazenar o número do CPF do cliente na plataforma ecommerce da Melhores Compras.Seu conteudo deve ser obrigatório. ';
...
COMMENT ON COLUMN mc_cli_juridica.dt_fundacao IS
    'Essa coluna ira armazenar data  de fundação do cliente na plataforma ecommerce da Melhores Compras.Seu conteudo deve ser obrigatório.';

COMMENT ON COLUMN mc_cli_juridica.nr_cnpj IS
    'Essa coluna ira armazenar o  numero do CNPJ do cliente na plataforma ecommerce da Melhores Compras.Seu conteudo deve ser obrigatório.';

COMMENT ON COLUMN mc_cli_juridica.nr_inscr_est IS
    'Essa coluna ira armazenar o  numero da Inscrição Estadual  do cliente na plataforma ecommerce da Melhores Compras.Seu conteudo deve ser opcional';
...
COMMENT ON COLUMN mc_cliente.nr_cliente IS
    'Essa coluna ira armazenar o codigo único do cliente na plataforma ecommerce da Melhores Compras.Seu conteudo deve ser obrigatório, único e preenhcido a  parrtir da chamada de sequence  SQ_MC_CLIENTE, a qual terá sempre o número disponivel para uso.';

COMMENT ON COLUMN mc_cliente.nm_cliente IS
    'Essa coluna ira armazenar o nome do cliente na plataforma ecommerce da Melhores Compras.Seu conteudo deve ser obrigatório.';

COMMENT ON COLUMN mc_cliente.qt_estrelas IS
    'Essa coluna ira armazenar a quantiade de estrelas do cliente na plataforma ecommerce da Melhores Compras.Seu conteudo deve ser obrigatório e ser possível de estar entre 1 e 5 estrelas.';

COMMENT ON COLUMN mc_cliente.vl_medio_compra IS
    'Essa coluna ira armazenar o valor  médio de gastos f eito pelo cliente na plataforma ecommerce da Melhores Compras.Seu conteudo deve ser obrigatório e deve ser calculado diariamente.';

COMMENT ON COLUMN mc_cliente.st_cliente IS
    'Essa coluna ira armazenar o stauts do cliente da Melhorees Compras. Os valores permitidos aqui são: A(tivo) e I(nativo).';

COMMENT ON COLUMN mc_cliente.ds_email IS
    'Essa coluna ira armazenar o email  do cliente da Melhorees Compras. No minimo é esperado um email contendo o caractere (@) em seu conteudo.';

COMMENT ON COLUMN mc_cliente.nr_telefone IS
    'Essa coluna ira armazenar o número do cliente da Melhorees Compras. A mascara de armazenamento deve ser: (<nr_ddd>) 99999-9999 e  deve ser utilizada pré definida.';

COMMENT ON COLUMN mc_cliente.nm_login IS
    'Essa coluna ira armazenar o login de cada cliente na plataforma ecommerce da Melhores Compras. Seu conteudo deve ser obrigatório e  único para cada cliente.';

COMMENT ON COLUMN mc_cliente.ds_senha IS
    'Essa coluna ira armazenar a senha de cada cliente na plataforma ecommerce da Melhores Compras.Seu conteudo deve ser obrigatório.';
...
COMMENT ON COLUMN mc_depto.cd_depto IS
    'Esta coluna ira receber o codigo do departamento  e seu conteudo é obrigatório.';

COMMENT ON COLUMN mc_depto.nm_depto IS
    'Esta coluna ira receber o nome do  departamento  e seu conteudo é obrigatório.';

COMMENT ON COLUMN mc_depto.st_depto IS
    'Esta coluna ira receber o status do  departamento  e seu conteudo é obrigatório. Os valores possíveis são: (A)tivo e (I)nativo.';
...
COMMENT ON COLUMN mc_end_cli.nr_cliente IS
    'Esta coluna ira receber o número do cliente e seu conteudo é obrigatório.';

COMMENT ON COLUMN mc_end_cli.cd_logradouro_cli IS
    'Esta coluna ira receber o codigo do logradouro  do cliente e seu conteudo é obrigatório.';
...
COMMENT ON COLUMN mc_end_cli.ds_complemento_end IS
    'Esta coluna ira receber o complemento do endereço do cliente e seu conteudo pode ser opcional.';
...
COMMENT ON COLUMN mc_funcionario.cd_funcionario IS
    'Esta coluna ira receber o codigo do funcionário e seu conteudo é obrigatório.';

COMMENT ON COLUMN mc_funcionario.cd_depto IS
    'Esta coluna ira receber o codigo do departamento onde o funcionario trabalha  e seu conteudo é obrigatório.';

COMMENT ON COLUMN mc_funcionario.cd_gerente IS
    'Esta coluna ira receber o codigo do gestor do funcionario trabalha  e seu conteudo é opcional.';

COMMENT ON COLUMN mc_funcionario.nm_funcionario IS
    'Esta coluna ira receber o nome do funcionário e seu conteudo é obrigatório.';

COMMENT ON COLUMN mc_funcionario.dt_nascimento IS
    'Esta coluna ira receber a data de nascimento  do funcionário e seu conteudo é obrigatório.';

COMMENT ON COLUMN mc_funcionario.fl_sexo_biologico IS
    'Esta coluna ira receber o sexo biológico do funcionário e seu conteudo é obrigatório.Os valores permitidos aqui seriam: (F)eminino; (M)asculino ou (Hermafrodita)';

COMMENT ON COLUMN mc_funcionario.ds_genero IS
    'Esta coluna ira receber o genero atribuido ao funcionário e seu conteudo é opcional.';

COMMENT ON COLUMN mc_funcionario.ds_cargo IS
    'Esta coluna ira receber o cargo do funcionário e seu conteudo é opcional.';

COMMENT ON COLUMN mc_funcionario.vl_salario IS
    'Esta coluna ira receber o valor do salário do funcionário e seu conteudo é opcional.';

COMMENT ON COLUMN mc_funcionario.ds_email IS
    'Esta coluna ira receber o email do funcionário e seu conteudo é opcional.';

COMMENT ON COLUMN mc_funcionario.st_func IS
    'Essa coluna ira armazenar o stauts do funcionário da Melhorees Compras. Os valores permitidos aqui são: A(tivo) e I(nativo).';
...
COMMENT ON COLUMN mc_funcionario.dt_desligamento IS
    'Data de desligamento  do Funcionario. Seu conteudo é opcional.';
...
COMMENT ON COLUMN mc_logradouro.cd_logradouro IS
    'Esta coluna ira receber o codigo do logradouro e seu conteudo é obrigatório.';

COMMENT ON COLUMN mc_logradouro.cd_bairro IS
    'Esta coluna ira receber o codigo do bairro e seu conteudo é obrigatório.';

COMMENT ON COLUMN mc_logradouro.nm_logradouro IS
    'Esta coluna ira receber o nome do logradouro e seu conteudo é obrigatório.';

COMMENT ON COLUMN mc_logradouro.nr_cep IS
    'Esta coluna ira receber o numero do CEP do Logradouro e seu conteudo é obrigatório.';
...
COMMENT ON COLUMN mc_produto.cd_produto IS
    'Essa coluna ira armazenar a chave primária da tabela de produtos da Melhorees Compras. A cada produto cadastrado será acionada a Sequence  SQ_MC_PRODUTO que se encarregará de gerar o próximo número único do produto.';

COMMENT ON COLUMN mc_produto.cd_categoria IS
    'Essa coluna ira armazenar a chave primária da tabela de categoria de produtos da Melhorees Compras. Cada categoria nova cadastrada  será acionada a Sequence  SQ_MC_CATEGORIA que se encarregará de gerar o próximo número único da categoria..';

COMMENT ON COLUMN mc_produto.nr_cd_barras_prod IS
    'Essa coluna ira armazenar o número do codigo de barras  do produto. Seu conteudo deve ser opcional.';

COMMENT ON COLUMN mc_produto.ds_produto IS
    'Essa coluna ira armazenar a descrição principal do produto. Seu conteudo deve ser  obrigatorio.';

COMMENT ON COLUMN mc_produto.vl_unitario IS
    'Essa coluna ira armazenar o valor unitário do produto. Seu conteudo deve ser > 0 ';

COMMENT ON COLUMN mc_produto.tp_embalagem IS
    'Essa coluna ira armazenar o tipo de embalagem do produto. Seu conteudo pode ser opcional.';

COMMENT ON COLUMN mc_produto.st_produto IS
    'Essa coluna ira armazenar o stauts do produto da Melhorees Compras. Os valores permitidos aqui são: A(tivo) e I(nativo).';

COMMENT ON COLUMN mc_produto.vl_perc_lucro IS
    'Essa coluna ira armazenar o percentual  do lucro médio para cada produto. Seu conteudo deve ser opcional.';

COMMENT ON COLUMN mc_produto.ds_completa_prod IS
    'Essa coluna ira armazenar a descrição completa do produto. Seu conteudo deve ser  obrigatorio.';
...
COMMENT ON COLUMN mc_sgv_produto_video.nr_sequencia IS
    'Essa coluna ira armazenar a sequencia do vídeo a partir do codigo do produto. Sempre o primeiro vídeo de determinado  produto deve iniciar com o número 1. Seu conteudo é obrigatório.';

COMMENT ON COLUMN mc_sgv_produto_video.cd_categoria IS
    'Essa coluna ira armazenar a chave primária da tabela de categoria de produtos da Melhorees Compras. Cada categoria nova cadastrada  será acionada a Sequence  SQ_MC_CATEGORIA que se encarregará de gerar o próximo número único da categoria..';

COMMENT ON COLUMN mc_sgv_produto_video.vd_produto IS
    'Essa coluna ira armazenar oo vídeo do produto no formato que melhor lhe  convier.  Para utilizar essa coluna tenha acesso a documentação oficial da Oracle. Seu conteudo é opcional.';

COMMENT ON COLUMN mc_sgv_produto_video.tp_video_prod IS
    'Essa coluna ira armazenar o tipo do  vídeo do produto. Seu conteudo é opcional.';

COMMENT ON COLUMN mc_sgv_produto_video.ds_path_video_prod IS
    'Essa coluna ira armazenar o caminho de acesso (link) contendo o  vídeo do produto. Seu conteudo é opcional.';

COMMENT ON COLUMN mc_sgv_produto_video.st_video_prod IS
    'Essa coluna ira armazenar o stauts do vídeo do produto da Melhorees Compras. Os valores permitidos aqui são: A(tivo) e I(nativo).';
...
COMMENT ON COLUMN mc_sgv_sac.nr_sac IS
    'Essa coluna ira armazenar a chave primária da tabela de SAC de vídeo  da Melhorees Compras. A cada SAC cadastrado pelo cliente será acionada a Sequence  SQ_MC_SGV_SAC que se encarregará de gerar o próximo número único do chamado SAC feito pelo Cliente.';
...
COMMENT ON COLUMN mc_sgv_sac.ds_detalhada_sac IS
    'Essa coluna  ira  receber a descrição completa do SAC aberto pelo cliente. Seu conteudo deve ser obrigatório.';

COMMENT ON COLUMN mc_sgv_sac.dt_abertura_sac IS
    'Essa coluna  ira  receber a data e horario do SAC aberto pelo cliente. Seu conteudo deve ser obrigatório.';

COMMENT ON COLUMN mc_sgv_sac.hr_abertura_sac IS
    'Essa coluna  ira  receber a hora do SAC aberto pelo cliente. Seu conteudo deve ser obrigatório.';

COMMENT ON COLUMN mc_sgv_sac.dt_atendimento IS
    'Essa coluna  ira  receber a data e horario do atendmiento SAC feita pelo funcionário da Melhores Compras. Seu conteudo deve ser opcional..';

COMMENT ON COLUMN mc_sgv_sac.hr_atendimento_sac IS
    'Essa coluna  ira  receber a hora do SAC do atendimento  feito  pelo funcionario da Melhores Compras. Seu conteudo deve ser opcional.';

COMMENT ON COLUMN mc_sgv_sac.nr_tempo_total_sac IS
    'Essa coluna  ira  receber o tempo total em horas  (HH24) computado desde a abertura até a conclusão dele. A unidade de medida é horas, ou seja, em quantas horas o chamado foi concluído desde a sua abertura.';

COMMENT ON COLUMN mc_sgv_sac.ds_detalhada_retorno_sac IS
    'Essa coluna  ira  receber a descrição detalhada do retorno feito pelo funcionário a partir da solicitação do cliente. Seu conteudo deve ser opcional e preenchido pelo funcionário.';

COMMENT ON COLUMN mc_sgv_sac.tp_sac IS
    'Essa coluna  ira  receber o TIPO  do chamado SAC aberto pelo cliente.  Seu conteudo deve ser  obrigatório e os possíveis valores são: (S)ugestão; (D)úvida ou  (E)logio.';

COMMENT ON COLUMN mc_sgv_sac.st_sac IS
    'Essa coluna  ira  receber o STATUS  do chamado SAC aberto pelo cliente.  Seu conteudo deve ser  obrigatório e os possíveis valores são: (E)m Atendimento; (C)ancelado; (F)echado ou (X)Fechado com Insatisfação do cliente.';

COMMENT ON COLUMN mc_sgv_sac.nr_indice_satisfacao IS
    'Essa coluna  ira  receber o índice de satisfação, , computado como um valor simples de 1 a 10, onde 1 refere-se ao cliente menos satisfeito e 10 o cliente mais satisfeito. Esse índice de satisfação é opcional e informado pelo cliente ao final do atendimento.';
...
COMMENT ON COLUMN mc_sgv_visualizacao_video.cd_visualizacao_video IS
    'Essa coluna ira armazenar a chave primária da tabela de visualização do vídeo, onde a cada click que o usuario logado ou anônimo realizar, será anotado o acesso realizado.';
...
COMMENT ON COLUMN mc_sgv_visualizacao_video.dt_visualizacao IS
    'Essa  coluna  ira  receber a data da visualização feita pelo  usuário no formato dd/mm/yyyy. Essa coluna é obrigatória.';

COMMENT ON COLUMN mc_sgv_visualizacao_video.nr_hora_visualizacao IS
    'Essa  coluna  ira  receber o horario no formato HH24 da visualização feita pelo  usuário. Essa coluna é obrigatória.';

COMMENT ON COLUMN mc_sgv_visualizacao_video.nr_minuto_video IS
    'Essa  coluna  ira  receber o horario do minuto do vídeo quando foi feita a visualização pelo  usuário. Essa coluna é obrigatória e os valores possíveis estão entre 00 e 59, representando os minutos.';

COMMENT ON COLUMN mc_sgv_visualizacao_video.nr_segundo_video IS
    'Essa  coluna  ira  receber o horario dos segundos do vídeo quando foi feita a visualização pelo  usuário. Essa coluna é obrigatória e os valores possíveis estão entre 00 e 59, representando os minutos.';
...
COMMENT ON COLUMN mc_sgv_ocorrencia_sac.nr_ocorrencia_sac IS
    'Essa coluna ira armazenar a chave primária da tabela ocorrência SAC, que será utilizada pela area de negócio da Melhores  Compras.';

COMMENT ON COLUMN mc_sgv_ocorrencia_sac.dt_abertura_sac IS
    'Essa coluna  ira  receber a data e horario do chamado  SAC aberto pelo cliente. Seu conteudo deve ser obrigatório e deve ser uma cópia dos dados da coluna DT_ABERTURA_SAC na  tabela MC_SGV_SAC.';

COMMENT ON COLUMN mc_sgv_ocorrencia_sac.ds_tipo_classificacao_sac IS
    'Essa coluna  ira  receber a descrição da classficação do tipo de chamado SAC. O tipo do chamado SAC aberto pelo cliente. Seu conteudo deve ser  obrigatório e os possíveis valores são: Sugestão; Dúvida ou  Elogio. Esses valores devem ser calculados a partir do conteudo da coluna TP_SAC na tabela MC_SGV_SAC.';

COMMENT ON COLUMN mc_sgv_ocorrencia_sac.cd_produto IS
    'Essa coluna ira receber o codigo do produto e seu conteudo deve ser obrigatório. Essa descrição  deve ser uma cópia dos dados da coluna CD_PRODUTO na tabela MC_PRODUTO.';

COMMENT ON COLUMN mc_sgv_ocorrencia_sac.ds_produto IS
    'Essa coluna ira receber a descrição do produto e seu conteudo deve ser obrigatório. Essa descrição  deve ser uma cópia dos dados da coluna DS_PRODUTO na tabela MC_PRODUTO.';

COMMENT ON COLUMN mc_sgv_ocorrencia_sac.vl_unitario_produto IS
    'Essa coluna ira armazenar o valor unitário do produto. Seu conteudo deve ser > 0 e  deve ser uma cópia dos dados da coluna VL_UNITARIO na tabela MC_PRODUTO.';

COMMENT ON COLUMN mc_sgv_ocorrencia_sac.vl_perc_lucro IS
    'Essa coluna ira armazenar o valor percentual  do lucro sobre o produto ofertado. Seu conteudo deve ser uma cópia dos dados da coluna VL_PERC_LUCRO na tabela MC_PRODUTO.';

COMMENT ON COLUMN mc_sgv_ocorrencia_sac.vl_unitario_lucro_produto IS
    'Essa coluna ira armazenar o valor do lucro unitario do lucro sobre o produto ofertado. Seu conteudo deve ser calculado sobre a seguinte formula: conteudo da coluna (VL_PERC_LUCRO / 100 ) * VL_UNITARIO_PRODUTO. Esses valores se  encontram na tabela MC_PRODUTO.';

COMMENT ON COLUMN mc_sgv_ocorrencia_sac.sg_estado IS
    'Esta coluna ira receber a siga do Estado. Esse conteudo é obrigatório e deve ser uma cópia dos dados da coluna SG_ESTADO  na tabela MC_ESTADO.';

COMMENT ON COLUMN mc_sgv_ocorrencia_sac.nm_estado IS
    'Esta coluna ira receber a descrição do nome  do Estado. Esse conteudo é obrigatório e deve ser uma cópia dos dados da coluna NM_ESTADO  na tabela MC_ESTADO.';

COMMENT ON COLUMN mc_sgv_ocorrencia_sac.nr_cliente IS
    'Essa coluna ira receber o codigo do nome do cliente  e seu conteudo deve ser obrigatório. Essa descrição  deve ser uma cópia dos dados da coluna NR_CLIENTE  na tabela MC_CLIENTE.';

COMMENT ON COLUMN mc_sgv_ocorrencia_sac.nm_cliente IS
    'Essa coluna ira receber o  nome do cliente  e seu conteudo deve ser obrigatório. Essa descrição  deve ser uma cópia dos dados da coluna NM_CLIENTE  na tabela MC_CLIENTE.';

COMMENT ON COLUMN mc_sgv_ocorrencia_sac.vl_icms_produto IS
    'Essa coluna ira armazenar o valor médio do ICMS pago sobre o produto ofertado a partir do Estado onde o cliente solicita a entrega. Seu conteudo deve ser calculado sobre a seguinte formula: conteudo da coluna (VL_PERC_ALIQUOTA_MEDIA / 100 ) * VL_UNITARIO_PRODUTO. O valor da alíquota é extraído da procedure prc_pega_aliquota_media_ICMS e o valor unitário do produto se  encontra na tabela MC_PRODUTO.';
...
-- Gabarito fase 3  (*inserindo algumas linhas no SAC)
...
VALUES (SQ_MC_SGV_SAC.NEXTVAL, 5, 15, 37, 'Cliente nao consegue ativar o mouse', sysdate-2, 14, 'D', 'E');
...
INSERT INTO MC_SGV_SAC(NR_SAC, NR_CLIENTE, CD_PRODUTO, CD_FUNCIONARIO, DS_DETALHADA_SAC,
DT_ABERTURA_SAC, HR_ABERTURA_SAC, TP_SAC, ST_SAC)
VALUES (SQ_MC_SGV_SAC.NEXTVAL, 10, 13, 38, 'O notebook trava após ligar', sysdate-2, 22, 'D', 'A');

UPDATE MC_SGV_SAC  SET DS_DETALHADA_RETORNO_SAC = 'O cliente fez o teste e o problema persiste',
DT_ATENDIMENTO = SYSDATE, HR_ATENDIMENTO_SAC = 2, NR_TEMPO_TOTAL_SAC = NR_TEMPO_TOTAL_SAC+2
WHERE NR_SAC = 2;

INSERT INTO MC_SGV_SAC(NR_SAC, NR_CLIENTE, CD_PRODUTO, CD_FUNCIONARIO, DS_DETALHADA_SAC,
DT_ABERTURA_SAC, HR_ABERTURA_SAC, TP_SAC, ST_SAC)
VALUES (SQ_MC_SGV_SAC.NEXTVAL, 6, 2, 37, 'O tenis está desbotado', sysdate-2, 22, 'S', 'A');

UPDATE MC_SGV_SAC  SET DS_DETALHADA_RETORNO_SAC = 'O cliente informou que é daltonico',
DT_ATENDIMENTO = SYSDATE, HR_ATENDIMENTO_SAC = 2, NR_TEMPO_TOTAL_SAC = NR_TEMPO_TOTAL_SAC+2
WHERE NR_SAC = 3;

INSERT INTO MC_SGV_SAC(NR_SAC, NR_CLIENTE, CD_PRODUTO, CD_FUNCIONARIO, DS_DETALHADA_SAC,
DT_ABERTURA_SAC, HR_ABERTURA_SAC, TP_SAC, ST_SAC)
VALUES (SQ_MC_SGV_SAC.NEXTVAL, 7, 20, 38, 'A TV não liga', sysdate-1, 14, 'D', 'A');

UPDATE MC_SGV_SAC  SET DS_DETALHADA_RETORNO_SAC = 'Depois do teste e setup o problema foi resolvido',
DT_ATENDIMENTO = SYSDATE, HR_ATENDIMENTO_SAC = 1, NR_TEMPO_TOTAL_SAC = NR_TEMPO_TOTAL_SAC+1
WHERE NR_SAC = 4;

INSERT INTO MC_SGV_SAC(NR_SAC, NR_CLIENTE, CD_PRODUTO, CD_FUNCIONARIO, DS_DETALHADA_SAC,
DT_ABERTURA_SAC, HR_ABERTURA_SAC, TP_SAC, ST_SAC)
VALUES (SQ_MC_SGV_SAC.NEXTVAL, 4, 8, 37, 'O calção é rosa', sysdate-1, 19, 'D', 'A');
